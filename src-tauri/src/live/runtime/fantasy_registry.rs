...
///
/// Entries are only cleared on encounter reset (`clear`), never on fantasy
/// dismiss: a buff that outlives its summon keeps resolving the remodel level
/// at publish time, matching the legacy registry behavior.
#[derive(Debug, Default)]
pub(crate) struct FantasyRegistry {
...
impl FantasyRegistry {
    pub(crate) fn register_summon(
        ...
        let resonance_skill_id = resolve_fantasy_skill_id(monster_id, marker_source_config_id);
...
pub(crate) fn resolve_fantasy_skill_id(
    monster_id: i32,
    marker_source_config_id: Option<i32>,
) -> Option<i32> {
...
fn normalize_fantasy_skill_id(source_config_id: i32) -> Option<i32> {
    ...
    let base_skill_id = source_config_id / 100;
...
mod tests {
    ...
    fn resolves_summoned_entity_source_without_skill_config() {
        ...
        registry.register_summon(EntityUuid(100), EntityUuid(1), 3_000_038, 5, None);

        assert_eq!(
            registry.resolve_remodel_level(Some(EntityUuid(100)), None),
            Some(5)
        );
    }
    ...
    fn resolves_character_source_by_fantasy_skill() {
        ...
        assert_eq!(
            registry.resolve_remodel_level(Some(EntityUuid(1)), Some(3_944)),
            Some(5)
        );
    }
    ...
    fn normalizes_skill_effect_source_config_id() {
        let mut registry = FantasyRegistry::default();
        registry.register_summon(EntityUuid(100), EntityUuid(1), 9_999_999, 4, Some(394_401));

        assert_eq!(
            registry.resolve_remodel_level(Some(EntityUuid(1)), Some(394_401)),
            Some(4)
        );
        assert_eq!(
            registry.resolve_remodel_level(Some(EntityUuid(1)), Some(3_944)),
    ...
    fn selects_the_matching_skill_for_the_same_summoner() {
        let mut registry = FantasyRegistry::default();
        registry.register_summon(EntityUuid(100), EntityUuid(1), 3_000_038, 5, None);
        registry.register_summon(EntityUuid(200), EntityUuid(1), 3_000_036, 2, None);
    ...
    fn leaves_unknown_character_source_unresolved() {
        ...
        assert_eq!(
            registry.resolve_remodel_level(Some(EntityUuid(1)), Some(999_999)),
            None
        );
        assert_eq!(
            registry.resolve_remodel_level(Some(EntityUuid(2)), Some(3_944)),
            None
        );
    }
}
